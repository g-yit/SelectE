...
# ==========================================================
# 新增加的代码，添加参数激活函数，初始化函数等
# selu,relu,leaky_relu,tanh,sigmoid,silu,softplus,gelu
parser.add_argument("--active_fn", default="relu", help="activation function for the model")
parser.add_argument("--init_fn", default="xavier_normal", help="activation function for the model")
# 两种融合方式，concat和add
# todo 后续可以考虑添加更多的融合方式
parser.add_argument("--fusion_mode",default="concat",help="fusion mode for the model")
# 修改了filter_size的传入方式，通过列表的方式可以自由的定义卷积方式，并且支持空洞卷积
parser.add_argument("--filter_size_list",default=[(1, 5, 1, 2), (3, 3), (1, 9)],help="卷积列表，格式为[(h,w,dh,dw),(h,w),(h,w)]，其中dh和dw表示空洞卷积的膨胀率，默认为1")
...
filter_size_list = parse_list_argument(args.filter_size_list)
# 数据集预处理和读取
...
model = SelectE(logger, vocab_size, embedding_dim=args.embedding_dim, input_drop=args.input_drop, hidden_drop=args.hidden_drop, feature_map_drop = args.feature_map_drop,
            k_w = args.k_w, k_h = args.k_h, output_channel= args.output_channel, filter_size_list=filter_size_list,
                 active_fn=args.active_fn, init_fn=args.init_fn)
device = args.device
model.init()
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(opt, 'min', factor=args.factor, min_lr=args.min_lr,
...
model = torch.load(args.save_name,weights_only=False)
