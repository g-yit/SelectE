...
class SelectE(torch.nn.Module):
    def __init__(self, logger, num_emb, embedding_dim=300, input_drop=0.4, hidden_drop=0.3, feature_map_drop=0.3,
        ...
        # 第二个3*3卷积，用不同的空洞比例 2
        self.se3_2 = SELayer(output_channel, reduction=int(0.5 * output_channel))
        # 第三个3*3卷积,用不同的空洞比例 5
        self.se3_3 = SELayer(output_channel, reduction=int(0.5 * output_channel))
        ...
        self.bn1 = torch.nn.BatchNorm2d(self.out_1 + self.out_2 * 3 + self.out_3)
        ...
        fc_length = self.reshape_H * self.reshape_W * (self.out_1 + self.out_2 * 3 + self.out_3)
    ...
    def forward(self, e1, rel):
        ...
        x1 = F.conv2d(x, f1, groups=e1_embedded.size(0),
                      padding=(int((self.h1 - 1) // 2), int((self.w1 - 1) // 2)))  # (4,48000,20,20)
        x1 = x1.reshape(e1_embedded.size(0), self.out_1, self.reshape_H, self.reshape_W)  # (128,128,20,20)
        x1 = self.bn1_1(x1)
        x1 = self.se1(x1)

        x3 = F.conv2d(x, f3, groups=e1_embedded.size(0),
        ...
        x3_2 = F.conv2d(x, f3, groups=e1_embedded.size(0),
                        padding=2, dilation=2)  # (1,2560,20,20)
        x3_2 = x3_2.reshape(e1_embedded.size(0), self.out_2, self.reshape_H, self.reshape_W)  # (128,20,20,20)
        x3_2 = self.bn1_2(x3_2)
        x3_2 = self.se3_2(x3_2)

        x3_3 = F.conv2d(x, f3, groups=e1_embedded.size(0),
                        padding=5, dilation=5)  # (1,2560,20,20)
        x3_3 = x3_3.reshape(e1_embedded.size(0), self.out_2, self.reshape_H, self.reshape_W)  # (128,20,20,20)
        x3_3 = self.bn1_2(x3_3)
        x3_3 = self.se3_3(x3_3)

        x5 = F.conv2d(x, f5, groups=e1_embedded.size(0),
        ...
        x = x1 + x3 + x3_2 + x3_3 + x5  # (128,20,20,20)
        y1, y3, y3_2, y3_3, y5 = self.att(x)
        y1 = y1.expand_as(x1)
        y3 = y3.expand_as(x3)
        y3_2 = y3.expand_as(x3_2)
        y3_3 = y3.expand_as(x3_3)
        y5 = y1.expand_as(x5)
        x1 = x1 * y1
        x3 = x3 * y3
        x3_2 = x3_2 * y3_2
        ...
        x = torch.cat([x1, x3, x3_2, x3_3, x5], dim=1)
        x = torch.relu(x)
